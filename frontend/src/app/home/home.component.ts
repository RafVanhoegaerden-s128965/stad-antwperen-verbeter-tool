...
export class HomeComponent {
  ...
  async onGeneralize() {
    ...
    try {
      ...
      if (this.currentRawTextId && this.currentSuggestionId) {
        ...
        await this.apiService.updateRawText(this.currentRawTextId, originalText, mediumType);
        const updatedSuggestions = await this.apiService.getSuggestion(this.currentSuggestionId);
        ...
        const suggestionResponse = await this.apiService.postSuggestion(rawTextId);
  ...
  private updateGeneratedText() {
    ...
    const sortedSuggestions = [...this.suggestions].sort((a, b) => 
      (b.info?.startPos ?? 0) - (a.info?.startPos ?? 0)
    );
